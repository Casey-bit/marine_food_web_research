...
def four(ax_four):
    ...
    # save_variable(familyUp,r'chugao\fish_220114_re\correlation_median_point\localDelay\familyUp.txt')
    # save_variable(familyDown,r'chugao\fish_220114_re\correlation_median_point\localDelay\familyDown.txt')
    ...
    for year in range(yearTotal - 10):
        for ran in range(3):
            ...
            aveDown[year, ran] = np.mean(aveFamilyDown[year: year + 10, ran][aveFamilyDown[year: year + 10, ran] > 0])
    for ran in range(3):
        ax_four.plot(np.arange(1975,2026-10,1),aveUp[:, ran],color = colors[2 * ran],linewidth = 3,label = 'Family shift into {}'.format(reg[ran]))
        ax_four.plot(np.arange(1975,2026-10,1),aveDown[:, ran],'--',linewidth = 3, color = colors[2 * ran + 1], label = 'Family shift out {}'.format(reg[ran]))
    ...
    ax_four.set_xticklabels(['1975-1987','1990-2002','2005-2017'],font1)
    ax_four.set_yticks([200,400,600,800,1000])
    # ax_four.set_ylabel('Average Family',font1)
    ax_four.set_yticklabels(ax_four.get_yticklabels())
    ax_four.set_xlabel('Year',font1)
    ax_four.set_xlim([1972,2017])
    ax_four.set_ylim([100,1000])
    ax_four.legend(prop = font2,ncol = 1,loc = 4,fontsize = 12, columnspacing = 0.5)
    # ax_four.set_title('Across Regions',font1)
    ax_four.text(1978,970,'Across Regions',verticalalignment="top",horizontalalignment="left",fontdict=font1)
    ax_four.text(1973, 970, 'B', verticalalignment="top",
