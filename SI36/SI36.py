...
year_idx_start = YEAR_START - 1970
year_idx_end   = YEAR_END   - 1970
...
for family in range(medianPoint.shape[0]):
    ...
    # 10 年块检查（固定块划分）
    ok_10yr = True
    for start_idx in [0, 10, 20, 30, 40]:
        sub = series[start_idx : start_idx + 10] if start_idx < 40 else series[40:]
        if len(sub[sub > 0]) == 0:
            ok_10yr = False
            break
