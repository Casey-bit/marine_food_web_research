...
# final_merge_df = pd.read_csv(r'by_family\final_merge_df.csv')
...
for k, single in g:
    ...
    # plt.subplot(121)
    # plt.plot(single['defined_year'], single['median'])
    ...
    # l.fillna(method='bfill', inplace=True)
    lat = l['lat'].values.tolist()
    # print(lat)
    for c in range(4):
        if not np.isnan(lat[c]):
            if int(lat[c] / 25) == int(lat[c + 1] / 25) and lat[c] < lat[c + 1]:
                mig_onerange_down[c][int(lat[c] / 25)].append(k)
            else:
...
x = [0.1 for i in range(9)]
y = [0.001,0.2,0.4,0.5,0.6,0.7,0.8,0.9,1]
color = ['rgba(238,103,76,0.5)' for i in range(9)]

x += [0.3 for i in range(9)]
y += [0.001,0.2,0.4,0.5,0.6,0.7,0.8,0.9,1]
color += ['rgba(238,103,76,0.6)' for i in range(9)]

x += [0.5 for i in range(9)]
y += [0.001,0.2,0.4,0.5,0.6,0.7,0.8,0.9,1]
