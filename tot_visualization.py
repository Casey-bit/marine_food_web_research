...
def sub_one(ax):
    ...
    # 数组大小的sample
    # nc1 = Dataset('H:\\oceanEnv\\carbon_ph_tem\\OceanSODA-ETHZ_GRaCER_v2021a_1982-2020.nc')
    # data = nc1.variables[variable][:]
    # long_tot = nc1.variables['lon'][:]  
    # lati_tot = nc1.variables['lat'][:]
    ...
    for i in range(2):
        ...
        aveplot[i] /= 9
        ax.plot(aveplot[i],lati_tot,style[i], color = (150/255,220/255,180/255,0.8),linewidth = 3,label = 'Chl-a (' + str(1998 + 9 * i) + " ~ " + str(1998 + 9 * i + 9) + ')')
    ...
    # plt.legend()
    # plt.title(variable)
    # aveplot = np.transpose(aveplot,(1,0))
    # np.savetxt('E:\\paper\\环境\\{}.csv'.format(variable), aveplot, delimiter = ',')    
    # # print(lati_tot[126])
    # aveYearLatitudeNorth = np.copy(aveYearLatitude[:,:126])
    # aveYearLatitudeNorth[np.isnan(aveYearLatitudeNorth)] = 0
    # a = []
    # for i in range(18):
    #     a.append(lati_tot[np.argmax(aveYearLatitudeNorth[i])])
    # plt.plot(np.arange(1998,2016,1),a)
    # plt.xticks(np.arange(1998,2016,1))

    # fig = plt.gcf()
    # fig.set_size_inches(18.5, 10.5)
    # fig.savefig('E:\\paper\\环境\\{}.png'.format(variable),dpi=200)
    # plt.show()