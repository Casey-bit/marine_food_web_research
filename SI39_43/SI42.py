...
df_raw = pd.read_csv('FamilyLat_q0.csv', header=None)
...
for (START, END) in windows:
    ...
    for family in range(medianPoint.shape[0]):
        ...
        block_size = 10
        block_starts = list(range(0, yearTotal, block_size))
        
        if block_starts[-1] + block_size > yearTotal:
            block_starts = block_starts[:-1]

        ok_10yr = True
        for start_idx in block_starts:
            sub = series[start_idx:start_idx + block_size]
            if len(sub[sub > 0]) == 0:
                ok_10yr = False
                break
    ...
    for idx in range(4):
        ...
        color_list = [(140 / 255, 185 / 255, 0 / 255), (20 / 255, 178 / 255, 255 / 255),
                      (255 / 255, 194 / 255, 102 / 255),
        ...
        if idx == 0:
            ...
            lg = plt.legend(
                ...
                handler_map={tuple: HandlerTuple(
                    ndivide=None), s[0]: custom_handler1, s[1]: custom_handler2, s[2]: custom_handler3,
                    s[3]: custom_handler4, s[4]: custom_handler5},
                prop=font1,
