...
def three(ax_three):
    ...
    # year family range
    percent = load_variavle(r'chugao\fish_220114_re\cluster\percent_2846.txt')
    ...
    # percentDown = [[[] for ran in range(3)] for year in range(yearTotal - 3)] # 记录三个区间每年比例上升下降的物种
    ...
    for ran in range(3):
        ...
        ax_three.bar(np.arange(1975,2026-12,1) + 0.3 * ran, aveUp[:, ran], color = colors[ran * 2],width = 0.3, label = 'Family shift into {}'.format(reg[ran]))
        ax_three.bar(np.arange(1975,2026-12,1) + 0.3 * ran, -aveDown[:, ran], color = colors[ran * 2 + 1], width = 0.3, label = 'Family shift out {}'.format(reg[ran]))
    font1 = {
         'weight': 'bold',
		 'style':'normal',
         'size': 15,
    ...
    ax_three.set_ylabel('Samples Proportion',font1,labelpad=15)
    ax_three.set_xticks([1975,1983,1991,1999,2007,2015])
    ax_three.set_yticks([-0.03,-0.02,-0.01,0,0.01,0.02,0.03])

    ax_three.set_xticklabels(['1970-1982','1978-1990','1986-1998','1994-2006','2002-2014','2010-2022'],font1)
    ...
    set_axis(ax_three)
    ax_three.text(1972.5, 0.0375, 'C', verticalalignment="top",
    ...
    for ran in range(3):
        for half in range(2):
            ...
            if half == 0:
                ax2.plot(np.arange(1975,2026-12,1), y_pred, color = colors[ran * 2], label ='P = ' + '%.4f' % get_p_value(initial, y_pred))
            else:
                ax3.plot(np.arange(1975,2026-12,1), y_pred, color = colors[ran * 2 + 1], label = 'P = ' +'%.4f' % get_p_value(initial, y_pred))
    ax2.legend(prop = font3,loc=1, fontsize = 10,labelspacing = 0.2)
    ax3.legend(prop = font3,loc=1, fontsize = 10,labelspacing = 0.2)
    ...
    ax2.text(1975,-0.003,'C$_1$ Fitting trends',verticalalignment="bottom",horizontalalignment="left",fontdict=font3)
    ax3.text(1975,-0.003,'C$_2$ Fitting trends',verticalalignment="bottom",horizontalalignment="left",fontdict=font3)
