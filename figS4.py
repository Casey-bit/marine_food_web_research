...
for item in range(4):
    ...
    if item == 3:
        ...
        cax.set_xticklabels(['1.13×10$^{-4}$','3.54×10$^{-4}$','6.26×10$^{-4}$','8.95×10$^{-4}$','1.16×10$^{-3}$','1.40×10$^{-3}$','1.62×10$^{-3}$','1.81×10$^{-3}$','1.97×10$^{-3}$','2.14×10$^{-3}$'],font1)
        cax.set_xlabel('Family Density: Distribution density of families in the total 559 species, 0.5° Latitude resolution and 1 Year resolution',font1)
    ...
    # print(len(my_kde.collections))
    # p = my_kde.collections[8].get_paths()[0]
    # v = p.vertices
    ...
    #将拟合后的结果打印
    # plt.scatter(kdex,kdey)
    ax.plot(x,y2,'r',label='Linear fitting (p < 0.01)')
...
fig.savefig(r'chugao\fish_220114_re\SM\figS3modecurve.png',dpi=1000)
plt.show()