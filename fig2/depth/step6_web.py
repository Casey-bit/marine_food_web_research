...
pvalue = load_variavle(r'fig2\depth\pvalue.txt')
familyExtract = load_variavle(r'fig2\depth\familyExtract.txt') # family name
...
web visulization
'''
cluster_inf = load_variavle(r'fig2\depth\cluster.txt')
...
for i in range(len(group)):
    for j in range(i + 1, len(group)):
        if correlation[group[i], group[j]] > 0.9 and (group[i],group[j]) not in G.edges and (group[j],group[i]) not in G.edges:
            ...
            # print(i, j, label)
output.to_csv(r'E:\gephi\data\vertex0.9_cluster_depth_230127.csv')
