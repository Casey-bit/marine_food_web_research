...
df = pd.read_csv(r'E:\gephi\data\vertex0.9_cluster_depth_230127.csv')
node = [] 
...
median = load_variavle(r'fig2\depth\median_denoising.txt')
...
for n, o in reserve:
    if n in node:
        ...
        if za > 1.96:
            if slope > 0:
                mk_test_res.loc[len(mk_test_res)] = [n, float(slope), float(za), 'Downward', 2]
            if slope < 0:
                mk_test_res.loc[len(mk_test_res)] = [n, float(slope), float(za), 'Upward', 1]
        else:
...
node_df = pd.read_csv(r'E:\gephi\data\vertex0.9_node_depth_230127.csv')
...
ax.set_xticklabels(['Total', 'Upward', 'Downward', 'Mixed'], font1)
ax.set_yticks([1, 2, 3, 4, 5])
ax.set_yticklabels([1, 2, 3, 4, 5], font1)
ax.text(0.6, 0.3 + 5.4 * 0.95, 'D', font3)
fig = plt.gcf()
fig.set_size_inches(8, 8)
fig.savefig(r'E:\paper\202301\fig\fig2B_depth.pdf', dpi=500)
