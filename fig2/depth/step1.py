...
for i in range(3):
    if i == 0:
        northern['range' + str(i + 1)] = pd.cut(northern['bathymetry'], [0, 30], labels=['(0,30]'])
    elif i == 1:
        northern['range' + str(i + 1)] = pd.cut(northern['bathymetry'], [30, 100], labels=['(30,100]'])
    elif i == 2:
...
family_year_median_df.to_csv(r'fig2\depth\family_year_median_df.csv')
