...
reserve1 = load_variavle(r'chugao\fish_220114_re\figure2\extra\reserve_level_oldindex.txt')
...
for year in range(yearTotal - 1):
    ...
    for family in range(2846):
        if family in [old for new,old in reserve]:
            ...
            level1 = [new for new,old in reserve1 if old == family]
            if len(level1):
                level = level1[0]
                for ran in range(3):
...
save_variable(family_into,r'chugao\fish_220114_re\figure2\extra\family_into_familylevel.txt')
save_variable(family_out_of,r'chugao\fish_220114_re\figure2\extra\family_out_of_familylevel.txt')
